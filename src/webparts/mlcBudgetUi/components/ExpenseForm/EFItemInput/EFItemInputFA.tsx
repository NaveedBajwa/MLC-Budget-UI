...
         </table>
    ...
    let response1 : any = this.GetFAItemCategoriesWS().then(
      response => {
        ...
        response.map(itemY=>{
          ...
          comOption.key = itemY.sdfa_item; 
          comOption.text = itemY.sdfa_item;
  ...
  public async GetFAItemCategoriesWS(): Promise<any[]> {
    let WSS = Constants.apiURL + '/GetAllFAItems';
    try{
...
public async UpdateItemWS()
{
  ...
  let itemText = "";
  if(this.state.SelectedFA != "Other" && this.state.SelectedFA != "")
  {
  ...
  public getItem(itemId): IComboBoxOptionLoan[]
  {
    ...
    let response1 : any = this.getItemWS(itemId).then(
      response => {
        ...
        else if(response1.ITEM_DESC != "Other")
        {
          innerItemDesc = "";
          innerItemcomboValue = response1.ITEM_DESC;
          textboxDisabled = true;
