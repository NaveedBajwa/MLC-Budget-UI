...
export interface IEFItemSectionFAProps {
...
export interface IEFItemSectionFAState {
  budgetCategoryText:string;
  budgetCategoryId:string;
  costCenterText:string;
  costCenterId:string;
...
export class EFItemSectionFA extends React.Component<IEFItemSectionFAProps, IEFItemSectionFAState> {
  
  public requestTotal:number;
  public approvedTotal:number;

  constructor(props: IEFItemSectionFAProps) {
    super(props);
...
                  />
  ...
  public async SetItemsWS(ItemCategoryKey): Promise<any[]> {
    ...
    let WSS = Constants.apiURL + '/GetFAItemsByBudgetCategory_CostCentre_FY?budgetCategory=' + this.state.budgetCategoryId + '&costCenter='+ 
        this.props.costCenterId +'&FY=' + this.props.budgetYearText ;
    try{
    return await this.props.budgetAppClient
    .get(WSS , AadHttpClient.configurations.v1)
