...
export interface IEFItemSectionSLProps {
...
export interface IEFItemSectionSLState {
  budgetCategoryText:string;
  budgetCategoryId:string;
  costCenterText:string;
  costCenterId:string;
...
export class EFItemSectionSL extends React.Component<IEFItemSectionSLProps, IEFItemSectionSLState> {
  
  public requestTotal:number;
  public approvedTotal:number;

  constructor(props: IEFItemSectionSLProps) {
    super(props);
...
                  />
  ...
  public async SetItemsWS(ItemCategoryKey): Promise<any[]> {
    ...
    let WSS = Constants.apiURL + '/GetSLItemsByBudgetCategory_CostCentre_FY?budgetCategory=' + this.state.budgetCategoryId + '&costCenter='+ 
        this.props.costCenterId +'&FY=' + this.props.budgetYearText ;
    try{
    return await this.props.budgetAppClient
    .get(WSS , AadHttpClient.configurations.v1)
