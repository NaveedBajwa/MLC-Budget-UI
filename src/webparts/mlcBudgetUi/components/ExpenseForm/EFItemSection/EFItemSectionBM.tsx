...
export interface IEFItemSectionBMProps {
...
export interface IEFItemSectionBMState {
  budgetCategoryText:string;
  budgetCategoryId:string;
  costCenterText:string;
  costCenterId:string;
...
export class EFItemSectionBM extends React.Component<IEFItemSectionBMProps, IEFItemSectionBMState> {
...
                  />
  ...
  public async SetItemsWS(ItemCategoryKey): Promise<any[]> {
    ...
    let WSS = Constants.apiURL + '/GetBMItemsByBudgetCategory_CostCentre_FY?budgetCategory=' + this.state.budgetCategoryId + '&costCenter='+ 
        this.props.costCenterId +'&FY=' + this.props.budgetYearText ;
    try{
    return await this.props.budgetAppClient
    .get(WSS , AadHttpClient.configurations.v1)
